...
class NICE(nn.Module):
    def __init__(self, prior, coupling, coupling_type,in_out_dim, hidden_dim, hidden_layers,device):
        ...
        if coupling_type == 'additive':
            ...
            odd = 1
            even = 0
            self.layer1 = AffineCoupling(in_out_dim, odd, _build_relu_network(half_dim, hidden_dim / 2, hidden_layers))
            self.layer2 = AffineCoupling(in_out_dim, even, _build_relu_network(half_dim, hidden_dim / 2, hidden_layers))
            self.layer3 = AffineCoupling(in_out_dim, odd, _build_relu_network(half_dim, hidden_dim / 2, hidden_layers))
            self.layer4 = AffineCoupling(in_out_dim, even, _build_relu_network(half_dim, hidden_dim / 2, hidden_layers))
            self.scaling_diag = Scaling(in_out_dim)
